...
const NoticeLetterPage: React.FC<{ onNavigate: (page: Page) => void; onBack: () => void; }> = ({ onNavigate, onBack }) => {
    return (
        <div className="animate-fade-in min-h-screen bg-slate-950 relative overflow-hidden selection:bg-indigo-500/30 selection:text-white">
             {/* Background Effects */}
            <div className="absolute top-[-10%] right-[-5%] w-[500px] h-[500px] bg-emerald-600/5 rounded-full blur-[100px] pointer-events-none"></div>
            <div className="absolute bottom-[-10%] left-[-5%] w-[500px] h-[500px] bg-teal-600/5 rounded-full blur-[100px] pointer-events-none"></div>
            ...
            <div className="shrink-0 relative z-40 pt-8 pb-4">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <h1 className="text-4xl md:text-5xl font-extrabold text-white tracking-tight mb-3">
                        Notices & <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">Letters</span>
                    </h1>
                     <p className="text-slate-400 text-lg max-w-2xl">Tools to draft various official notices, circulars, and letters for all your communication needs.</p>
                </div>
            </div>
            <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
                    <GeneratorCard
                        ...
                        icon={getLargeGeneratorIcon('general_notice')}
                        title="Notice"
                        description="Create and format general notices for display on notice boards or for circulation."
                        onClick={() => onNavigate('general_notice')}
                    />
                    <GeneratorCard
                        style={{ animationDelay: '200ms' }}
                        className="animate-slide-in-up"
                        icon={getLargeGeneratorIcon('noc')}
                        title="No Objection Certificate (NOC)"
                        description="Generate official No Objection Certificates for various purposes like travel, employment, or education."
                        onClick={() => {}}
                        disabled
                    />
                </div>
...
export default NoticeLetterPage;